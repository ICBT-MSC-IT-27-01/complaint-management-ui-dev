  ...
  template: `
    <div class="page-header">
      ...
      <div class="d-flex gap-2">
        <button class="btn btn-outline-primary" (click)="exportSummaryPdf()"><i class="bi bi-file-earmark-pdf me-1"></i> Export PDF</button>
    ...
    <div class="cms-filterbar mb-3">
      <div class="row g-2 align-items-end">
        ...
        <div class="col-auto">
          ...
          <button class="btn btn-link text-decoration-none" (click)="filter={}; load()">Clear</button>
          <button class="btn btn-outline-primary" (click)="loadHighPriorityOverdue()"><i class="bi bi-exclamation-diamond me-1"></i>High Priority</button>
        </div>
      </div>
    </div>
    ...
    <ng-container *ngIf="summary() && !loading()">
      <div class="cms-kpi-grid">
        <div class="kpi-card kpi-blue"><div class="kpi-body"><div class="kpi-label">Total Complaints</div><div class="kpi-value">{{ summaryMetric('totalComplaints') }}</div></div></div>
        <div class="kpi-card kpi-orange"><div class="kpi-body"><div class="kpi-label">Avg Resolution</div><div class="kpi-value">{{ summaryMetric('avgResolutionHours') }}h</div></div></div>
        <div class="kpi-card kpi-green"><div class="kpi-body"><div class="kpi-label">Compliance Score</div><div class="kpi-value">{{ summaryMetric('slaCompliancePercent') }}%</div></div></div>
        <div class="kpi-card kpi-blue"><div class="kpi-body"><div class="kpi-label">Pending Reviews</div><div class="kpi-value">{{ summaryMetric('pendingReviews') }}</div></div></div>
      </div>
